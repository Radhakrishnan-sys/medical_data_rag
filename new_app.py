...
from rag_experiment_tracker import init_experiment_log, log_experiment
...
# =====================
def running_agent():
    init_experiment_log()   # <--- Make sure this is called once at the very start
 
    # Define experiment config at the start of your session/run
    config = {
        "experiment_name": "gpt4o-chroma-minilm-512chunks",   # change as you experiment
    ...
    while True:
        ...
        if response_msg:
            ...
            # FEEDBACK LOOP: Prompt user for feedback (optional)
            feedback = input("\nRate the answer (1-5) or press enter to skip: ").strip()
            notes = input("Any comments? (press enter to skip): ").strip()
 
            # METRICS
            metrics = {
                "user_rating": int(feedback) if feedback.isdigit() else "",
                "latency": latency,
            }
 
            # LOG THE EXPERIMENT
            log_experiment(config, metrics, notes)
